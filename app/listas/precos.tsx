...
export default function PrecosScreen() {
  ...
  // Primeira entrada na área: pergunta a loja antes de qualquer outra coisa.
  useEffect(() => {
  ...
  const handleRemovePrice = (itemId: string, marketId: string, productName: string, marketName: string) => {
    showConfirm(
      {
        title: 'Remover preço',
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top + 15 }]}>
        ...
        <Text style={styles.headerTitle}>Preços</Text>
        <View style={styles.backBtnSpacer} />
      </View>

      {/* Mercado atual — trocar acessível de qualquer ponto da área */}
      <TouchableOpacity
          ...
          {tab === 'mercados'
                ...
                <View key={group.marketId} style={styles.card}>
                  ...
                  {group.entries.map((entry) => (
                    <TouchableOpacity
                      ...
                      style={styles.row}
                      onLongPress={() =>
                        handleRemovePrice(entry.item.id, group.marketId, entry.item.productName, group.marketName)
                      }
                      delayLongPress={500}
                      activeOpacity={0.7}
                    >
            ...
            : (
              <>
                ...
                {items.filter((item) => item.prices.length === 1).length > 0 && (
                  <View style={styles.card}>
                    <Text style={styles.cardTitle}>Só um preço registrado</Text>
                    <Text style={styles.cardHint}>
                      Escaneie em outro mercado pra poder comparar.
                    </Text>
                    {items
